...
# Both named account journeys must run on each destination. A skipped or empty
# suite can otherwise make xcodebuild return success without testing the flows.
if ! jq -e '
  .result == "Passed"
  and .totalTestCount == 2
  and .passedTests == 2
  and .failedTests == 0
  and .skippedTests == 0
...
' "$run_dir/summary.json" > /dev/null; then
  ...
  echo "Expected both UI journeys to pass with zero skips on $device_name." >&2
  exit 1
fi
