...
# All eleven named account journeys must run on each destination. A skipped or empty
...
' "$run_dir/summary.json" > /dev/null; then
  ...
  echo "Expected all eleven UI journeys to pass with zero skips on $device_name." >&2
  exit 1
fi
